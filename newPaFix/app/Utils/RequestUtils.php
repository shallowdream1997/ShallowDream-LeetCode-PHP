...
class RequestUtils
{
    ...
    public function getUserSheetByUserName($userName)
    {
        ...
        $get = $redisService->hGet(RedisKeys::USER_NAME, $userName);
        $userInfo = [];
        if (empty($get)) {
            //缓存不存在就读表并写入redis
            ...
            if (!empty($resp)){
                if ($resp['status'] == '启用'){
                    ...
                    $redisService->hSet(RedisKeys::USER_NAME, $userName, json_encode($userInfo, JSON_UNESCAPED_UNICODE), 60 * 60 * 12);
                }
            }
    ...
    public function getEmployeeByCompanySequenceId(string $companySequenceId)
    {
        ...
        foreach ($resp as $info){
            if ($info['status'] == '启用'){
                ...
                $redisService->hSet(RedisKeys::USER_NAME, $info['userName'], json_encode($userInfo, JSON_UNESCAPED_UNICODE), 60 * 60 * 72);
            }
        }
        return $this;
    }
    ...
    public function returnEmployeeByCompanySequenceId()
    {
        ...
        $redisService = new RedisService();
        $get = $redisService->hGetAll(RedisKeys::USER_NAME);
    ...
    {
        $redisService = new RedisService();
        $get = $redisService->hGet(RedisKeys::CATEGORY_ID, $categoryId);
        $categoryIdInfo = [];
        if (empty($get)) {
            ...
            $redisService->hSet(RedisKeys::CATEGORY_ID, $categoryId, json_encode($categoryIdInfo, JSON_UNESCAPED_UNICODE), 60 * 60 * 1);
        }else{
