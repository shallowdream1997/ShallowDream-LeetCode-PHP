...
class ProductSkuController
{
    ...
    private string $module = "platform-wms-application";
    public function __construct(string $port = 'test')
    {
        $this->log = new MyLogger("product_sku");
        $this->requestUtils = new RequestUtils($port);
    }
    public function getModule(string $module): self{
        switch ($module){
            case "wms":
    ...
    public function getXlsxByFile(string $file = "")
    {
        $excelUtils = new ExcelUtils();
        $fileName = STORAGE_PATH . "/exports/";
        switch ($file) {
...
//            }
    ...
    public function getPmoData($downloadOssLink,$list = [],$env = "test"){
        if (count($list) == 0){
            $fileContent = (new ExcelUtils())->getXlsxData(STORAGE_PATH . "/exports/PMO.xlsx");
